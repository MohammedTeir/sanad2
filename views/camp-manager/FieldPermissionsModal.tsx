// views/camp-manager/FieldPermissionsModal.tsx
...
const FIELD_GROUPS: FieldGroup[] = [
  ...
  },
  {
    ...
    icon: <Icons.User />,
    fields: [
      // Spouse's basic info (gender-neutral - wife for male heads, husband for female heads)
...
const FieldPermissionsModal: React.FC<FieldPermissionsModalProps> = ({ familyId, onClose }) => {
  ...
  const loadPermissions = async () => {
    try {
      ...
      perms.forEach(p => {
        permsMap[p.field_name] = p.is_editable;
      });
      setPermissions(permsMap);
    } catch (error: any) {
      console.error('Error loading permissions:', error);
      setToast({ message: 'فشل تحميل الصلاحيات', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeselectAll = () => {
    // Set all fields to false (not empty object)
    const newPermissions: { [key: string]: boolean } = {};
    FIELD_GROUPS.forEach(group => {
      group.fields.forEach(f => {
  ...
  const handleSave = async () => {
    try {
      ...
      await realDataService.bulkUpdateFieldPermissions(familyId, permissionsArray);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" dir="rtl">
      <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        {/* Header */}
        <div className="bg-gradient-to-l from-emerald-600 to-emerald-700 p-6 text-white flex-shrink-0">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-black mb-1">صلاحيات الحقول القابلة للتعديل</h2>
              <p className="text-emerald-100 text-sm font-bold">
                حدد الحقول التي يمكن للمستفيد تعديلها
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
            <div className="relative flex-1 w-full">
              <input
                ...
                className="w-full px-4 py-2.5 pr-10 rounded-xl border-2 border-gray-200 focus:border-emerald-500 focus:outline-none font-bold text-sm"
              />
                ...
                <div key={group.id} className="bg-white rounded-2xl border-2 border-gray-100 overflow-hidden">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {status === true && (
                        <span className="text-xs text-emerald-600 font-bold">الكل محدد</span>
                      )}
                      {status === 'some' && (
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {group.fields.map((fieldItem) => (
                          <label
                            ...
                            style={{
                              borderColor: permissions[fieldItem.field] ? '#059669' : '#e5e7eb',
                              backgroundColor: permissions[fieldItem.field] ? '#ecfdf5' : 'white'
                            }}
                          >
                            ...
                            <div className="relative">
                              ...
                              <div
                                className={`w-12 h-6 rounded-full transition-colors ${
                                  permissions[fieldItem.field] ? 'bg-emerald-600' : 'bg-gray-300'
                                }`}
                              >
                                <div
        ...
        <div className="p-4 border-t bg-gray-50 flex-shrink-0">
          <div className="flex flex-col sm:flex-row gap-3 justify-between items-center">
            <p className="text-sm text-gray-600 font-bold">
              {Object.values(permissions).filter(Boolean).length} حقول قابلة للتعديل
            </p>
            <div className="flex gap-3 w-full sm:w-auto">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex-1 sm:flex-none px-6 py-3 bg-gradient-to-l from-emerald-600 to-emerald-700 text-white rounded-xl font-bold hover:from-emerald-700 hover:to-emerald-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {saving ? (
                  <>
...
export default FieldPermissionsModal;
