...
const DPManagement: React.FC = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet('سجل العائلات المركزي', { views: [{ rightToLeft: true }] });

      const statusText = activeTab === 'all' ? 'جميع العائلات' : activeTab === 'موافق' ? 'العائلات المعتمدة' : 'عائلات قيد الانتظار';

      // 1. Add Title Row
      const titleRow = worksheet.addRow([`تقرير السجل المركزي للنازحين (${statusText}) - الإدارة العامة`]);
      titleRow.height = 35;
      ...
      filteredDPs.forEach((dp, index) => {
        ...
        // 6. Orphan count (derived from members if orphan flag exists)
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
            <button
              onClick={() => {
                ...
                const statusLabel = activeTab === 'all' ? 'كامل_السجل' : activeTab.replace(' ', '_');
                setExportFileName(`السجل_المركزي_${statusLabel}_${dateStr}`);
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100">
                <p className="text-sm font-bold text-emerald-800 leading-relaxed">
                  سيتم تصدير السجل المركزي لجميع المخيمات مع كافة التفاصيل (الهوية، المخيم، الهاتف، السكن، الإحصائيات، الحالات الصحية).
                </p>
              </div>
            </div>
